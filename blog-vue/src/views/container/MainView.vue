        ...
        <div>
                ...
                <el-card>
                    ...
                    <el-tree style="line-height: normal;margin-top: -35px;" :data="navList" ref="tree"  
                        ...
                        <template v-slot="{ node }">
                            <span v-if="isSelect(node.data.name)" style="font-size: 15px;font-weight:bold;color: black;">{{ node.data.name }} ({{ node.data.number }})</span>
            ...
            <el-card class="box-card-main" id="content" v-for="article in articleList" :key="article.id">
                <el-button type="text" class="articleBt">
                    <h2 style="line-height: 30%;" @click="getArticleContextByTitle(article.title, article.categoryName)">{{
                        article.title }}</h2>
                </el-button><br>
                <i class="el-icon-s-order"></i><span style="font-size: 16px;">&nbsp;&nbsp;文章简介：{{ article.intro | truncate(15) }}
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
                <div style="float: right;">
                        <i class="el-icon-date"></i><span>&nbsp;&nbsp;{{
                            article.createTime.split('T')[0] }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>

                        <i class="el-icon-folder-opened" ></i><span style="display:inline-block;width: 95px;">&nbsp;&nbsp;{{ article.categoryName }}</span>
