...
public class ArticleController {
    ...
    public Result<PageResult> getArticlesByCategoryNameAndPage(@PathVariable(required = false) String categoryName, @RequestParam(required = false) Integer pageSize, @RequestParam(required = false) Integer currentPage ){
        log.info("根据分类名称和每页大小以及第几页获取文章信息:{},{},{}",categoryName,pageSize,currentPage);
        PageResult pageResult  = articleService.getArticlesByCategoryNameAndPage(categoryName,pageSize,currentPage);
    ...
    public Result<PageResult> getArticlesBySearch(@PathVariable String searchValue,@RequestParam(required = false) Integer pageSize,@RequestParam(required = false) Integer currentPage){
    ...
    public Result<UserLike> getUserLike(@RequestBody UserLikeDTO userLikeDTO){
        log.info("获取用户的点赞相关信息：{}",userLikeDTO);
        UserLike userLike = articleService.getUserLike(userLikeDTO);
